...
const Chat = ({ setUserCount, username }: Props) => {
    ...
    useEffect(() => {
        ...
        // Handle incoming public messages (ignore room-scoped ones)
        socket.on('message', (message: any) => {
            if ((message?.roomName || '').trim()) return;
    ...
    const handleSendMessage = () => {
        ...
        // Public/room message flow
        if (socket) {
            socket.emit('message', { text: messageInput, userId: socket.id, roomName: roomName || undefined });
            setMessageInput('');
            setInputError('');
        }
    };
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (socket) {
            // Is Typing Logic
            socket.emit('typing', { username, roomName: roomName || undefined })
        }

        clearTimeout(typingTimeout)

        typingTimeout = setTimeout(() => {
            socket?.emit('stopTyping', { username, roomName: roomName || undefined })
        }, 5000)
    };

    // moved to Rooms component
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-[minmax(240px,1fr)_3fr] gap-4">
            ...
            <div className="flex flex-col">
                <Rooms socket={socket} currentRoom={roomName} onChangeRoom={(r) => {
                    setRoomName(r);
                    setSelectedUserId(null);
                }} />

                {roomName.trim() ? (
                    <RoomChat socket={socket} roomName={roomName.trim()} username={username} />
                ) : (
                    <>
                        {/* Send mode banner */}
                        <div className="mb-2">
