...
interface AppWithDBProps {
  onBackToSimple: () => void;
}

const AppWithDB: React.FC<AppWithDBProps> = ({ onBackToSimple }) => {
  const socket = useSocket();
  const { user: currentUser, token, isAuthenticated, login, logout, loading } = useAuth();
      ...
      const loadRooms = async () => {
        try {
          const [userRoomsData, allRoomsData] = await Promise.all([
            authService.getUserRooms(token),
            authService.getAllRooms(token)
          ]);
